...
*/
	...
	function syncUsers(){
		...
		while($user = mysql_fetch_assoc($res)){
			$sql = "Insert into participant (pc_tatid, pc_name, pc_email, pc_college, pc_contact, pc_state, pc_gender, pc_accomreqst) values('".pg_escape_string($user['tathva_id'])."','".pg_escape_string($user['name'])."','".pg_escape_string($user['email'])."','".pg_escape_string($user['college_name'])."','".pg_escape_string($user['phone'])."','".pg_escape_string($user['state'])."','','n');<br />";
			echo $sql;
		}
	}
